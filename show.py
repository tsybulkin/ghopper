...
def show(anim_file, state_log,tau):
	...
	draw_line(f, body_points(state_log), 9, 'blue',tau)
...
def body_points(state_log):
	bp = []
	for (x,y,a,b,g) in state_log:
		xyh = np.array([x,y]) + L*np.array([cos(a), sin(a)]) + L*np.array([cos(a+b),sin(a+b)])
		bp.append([xyh + Lb/2 * np.array([cos(a+b-g),sin(a+b-g)]), 
				   xyh - Lb/2 * np.array([cos(a+b-g),sin(a+b-g)])])
	return bp
